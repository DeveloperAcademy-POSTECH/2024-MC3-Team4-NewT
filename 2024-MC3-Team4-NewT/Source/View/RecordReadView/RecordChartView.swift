...
struct RecordChartView: View {
    ...
    func getPinnedRecords() -> [OldSurfingRecordOne] {
        ...
        let pinnedStartTimes = pinnedStartTimesStrings.compactMap { DateFormatterManager.shared.detailedDateTimeFormatter.date(from: $0) }
        ...
        let filteredRecords = surfingRecordOneData.filter { item in
            pinnedStartTimes.contains { pinnedTime in
                DateFormatterManager.shared.detailedDateTimeFormatter.string(from: pinnedTime) == DateFormatterManager.shared.detailedDateTimeFormatter.string(from: item.surfingStartTime)
            }
        }
